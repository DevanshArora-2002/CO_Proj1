...
def decimal_to_binary(num):
    ...
    if(int(num)<0 or int(num)>255):
        raise error.ImmediateException("Illegal immediate values")
...
def func_B_C(instruction):
    ...
    if (instruction[0] == "mov" and instruction[2][0] == "$"):  # type B
        opcode = "00010"
        encoding = typeB(opcode, reg_address[instruction[1]], decimal_to_binary(instruction[2][1:]))
